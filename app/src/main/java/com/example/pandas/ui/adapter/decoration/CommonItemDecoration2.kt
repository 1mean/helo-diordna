 ...
 * @version: v1.0
 */
public class CommonItemDecoration2(
    private val hasTop: Boolean = false,//是否有单独的top
    private val spanCount: Int = 1,
    private val paddingTop: Int = 0,//垂直方向的padding
    private val paddingSides: Int = 0, //水平方向的padding
    private var paddingMid: Int = 0,
    ...
    override fun getItemOffsets(
        ...
        if (hasTop && position == 0) {
            outRect.set(paddingSides, paddingTop, paddingSides, paddingBottom)
        } else {
            //最左边和最右边的padding和中间位置的padding不同
            if (columnIndex == 0) {
                outRect.set(paddingSides, paddingTop, paddingMid, paddingBottom)
            } else if (columnIndex == (spanCount - 1)) {
                outRect.set(paddingMid, paddingTop, paddingSides, paddingBottom)
            } else {
